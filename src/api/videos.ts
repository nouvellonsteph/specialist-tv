...
// ...
  ...
  // Perform advanced vector search with filters
  async performAdvancedVectorSearch(
    query: string,
    limit: number,
    minScore: number,
    filters: {
    ...
    try {
      ...
      const vectorResults = await this.performDetailedVectorSearch(query, limit * 3, minScore);
      
      if (vectorResults.length === 0) {
        return [];
      }

      // Get unique video IDs from vector results
      const videoIds = [...new Set(vectorResults.map(r => r.videoId))];
      
      // Fetch full video details for filtering
      ...
      const validVideos = videoDetails.filter((video): video is Video => video !== null);
      ...
      if (filters.duration.min || filters.duration.max) {
        filteredVideos = filteredVideos.filter(video => {
          ...
          if (filters.duration.min && video.duration < filters.duration.min) return false;
          if (filters.duration.max && video.duration > filters.duration.max) return false;
          return true;
        });
      }
      ...
      if (filters.tags.length > 0) {
        const videoTagsPromises = filteredVideos.map(async (video) => {
          const tags = await this.getVideoTags(video.id);
          return { video, tags: tags.map(t => t.name) };
        });
        
        const videoTagsResults = await Promise.all(videoTagsPromises);
        filteredVideos = videoTagsResults
          .filter(({ tags }) => filters.tags.some(filterTag => tags.includes(filterTag)))
          .map(({ video }) => video);
      }
      
      // Creators filter (if we had creator info in the database)
      if (filters.creators.length > 0) {
        filteredVideos = filteredVideos.filter(video => 
          video.created_by && filters.creators.includes(video.created_by)
        );
      }
      
      // Filter vector results to only include filtered videos
      const filteredVideoIds = new Set(filteredVideos.map(v => v.id));
      let filteredVectorResults = vectorResults.filter(result => 
        filteredVideoIds.has(result.videoId)
      );
      ...
      if (filters.sortBy !== 'relevance') {
        ...
        filteredVectorResults = filteredVectorResults.sort((a, b) => {
          ...
          switch (filters.sortBy) {
            case 'date':
              comparison = new Date(videoA.upload_date).getTime() - new Date(videoB.upload_date).getTime();
              break;
            case 'duration':
              comparison = (videoA.duration || 0) - (videoB.duration || 0);
              break;
            case 'views':
              comparison = (videoA.view_count || 0) - (videoB.view_count || 0);
              break;
            case 'title':
      ...
      return filteredVectorResults.slice(0, limit);
  ...
  // Get all available creators for filtering
  async getAvailableCreators(): Promise<string[]> {
    try {
      const result = await this.env.DB.prepare(`
        SELECT DISTINCT created_by FROM videos 
        WHERE created_by IS NOT NULL 
        ORDER BY created_by ASC
      `).all();
      
      return result.results.map(row => String(row.created_by));
    } catch (error) {
      console.error('Failed to get available creators:', error);
      return [];
    }
  }
